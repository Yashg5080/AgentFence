...
export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => ({})) as { message?: unknown; protectedMode?: unknown };
  ...
  return NextResponse.json(await replyToSupportMessage(message, body.protectedMode === true));
}
