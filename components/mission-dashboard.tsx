...
export function MissionDashboard() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <section className="attack-matrix"><div className="section-head"><div><p className="eyebrow">Mission coverage</p><h2>Three independent paths. One deterministic verdict.</h2></div><span className="run-id">{safe ? "3 / 3 BLOCKED" : run ? "3 / 3 PROVEN" : "READY"}</span></div><div className="matrix-rows">{attackKinds.map((kind, index) => { const item = runs.find(candidate => candidate.attackKind === kind); return <div className={`matrix-row ${item?.outcome === "blocked" ? "matrix-safe" : item ? "matrix-risk" : ""}`} key={kind}><span className="matrix-index">0{index + 1}</span><strong>{labels[index]}</strong><span>{item ? item.outcome === "blocked" ? "Blocked before tool execution" : "Sensitive tool executed" : "Awaiting mission"}</span><b>{item ? item.outcome === "blocked" ? "BLOCKED" : "EXPOSED" : "QUEUED"}</b></div>; })}</div></section>
