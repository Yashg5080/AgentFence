...
export function MissionDashboard() {
  ...
  const auditEvents = !run ? [] : safe ? [
    ["01", "Exploit replayed", "The original untrusted input was replayed unchanged.", "done"],
    ["02", "Policy evaluated", "Verified-ticket and bulk-export rules were enforced.", "done"],
    ["03", "Sensitive call stopped", "customer_lookup() was denied before execution.", "done"],
    ["04", "Regression suite passed", "All three synthetic attacks are now blocked.", "done"],
  ] : [
    ["01", "Untrusted input received", "The support agent accepted an attack instruction.", "risk"],
    ["02", "Authorization missing", "No verified ticket or least-privilege scope was required.", "risk"],
    ["03", "Sensitive tool executed", "customer_lookup() returned synthetic email fields.", "risk"],
    ["04", "Patch required", "Generate a guardrail and replay the same attack.", "pending"],
  ];
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <section className="review-grid"><section className="card patch-card"><div className="section-head"><div><p className="eyebrow">Developer-ready patch</p><h2>Review the authorization diff</h2></div><span className="review-tag">{guardrail ? "READY TO COPY" : "AWAITING PATCH"}</span></div>{guardrail ? <div className="code-diff"><code><span className="diff-remove">- customer_lookup(query, &#123; fields: ["name", "email"] &#125;)</span><span className="diff-add">+ requireVerifiedTicket(context.ticketId)</span><span className="diff-add">+ denyBulkEmailFields(requestedFields)</span><span className="diff-add">+ customer_lookup(scopedCustomerId, &#123; fields: ["name"] &#125;)</span></code><p>{guardrail.explanation}</p></div> : <div className="patch-placeholder">Generate the guardrail to turn the security finding into a reviewable, least-privilege code change.</div>}</section><section className="card audit-card"><div className="section-head"><div><p className="eyebrow">Audit trail</p><h2>Decision timeline</h2></div><span className="review-tag">{run ? "4 EVENTS" : "WAITING"}</span></div>{auditEvents.length ? <div className="audit-events">{auditEvents.map(([id, title, detail, tone]) => <div className={`audit-event audit-${tone}`} key={id}><span>{id}</span><div><strong>{title}</strong><p>{detail}</p></div></div>)}</div> : <div className="patch-placeholder">Run a mission to collect a sanitized, timestamp-free security trail.</div>}</section></section>
      <section className="safe-tools"><div><p className="eyebrow">Least-privilege tools</p><h2>Normal support remains useful</h2><p>These low-risk actions work without customer-email access. Try one in the chat.</p></div><div className="safe-tool-list"><button onClick={() => setDraft("Where is order ORD-1042?")}><span><Icon name="check" /></span><strong>order_status()</strong><small>Scoped order update</small></button><button onClick={() => setDraft("Create a support ticket for my delivery issue")}><span><Icon name="check" /></span><strong>create_support_ticket()</strong><small>Minimal issue summary</small></button><button onClick={() => setDraft("Look up customer record with ticket ACME-2048")}><span><Icon name="lock" /></span><strong>customer_lookup()</strong><small>Verified ticket required</small></button></div></section>
      <section className={`pr-card ${safe ? "pr-safe" : ""}`}><div className="pr-heading"><span className="pr-icon"><Icon name={safe ? "check" : "spark"} /></span><div><p className="eyebrow">Delivery preview</p><h2>{safe ? "Mock pull request ready to merge" : "Patch will create a security pull request"}</h2><p>{safe ? "Policy update and regression tests are passing in the simulated CI workflow." : "Generate the patch to prepare a developer-ready policy and regression-test change."}</p></div></div><div className="pr-checks"><span><Icon name={safe ? "check" : "alert"} />Policy guardrail</span><span><Icon name={safe ? "check" : "alert"} />Regression replay</span><span><Icon name={safe ? "check" : "alert"} />No email exposure</span></div></section>
